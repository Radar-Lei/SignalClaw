...
上图把事件结果汇总成三个视角：`E1/E2/M1` 上的 emergency delay、`B1/B2` 上的 person-delay，以及全部事件场景上的 average delay。由于 FixedTime 与其他方法差距过大，左侧和中间面板使用了对数坐标，便于直接比较。
...
- `evoprog/` 是核心代码
- `scripts/` 是实验入口和配置
- `figures/` 保存可复现的绘图脚本、数据文件和论文图 PDF
- `images/src/` 保存论文原始图源
